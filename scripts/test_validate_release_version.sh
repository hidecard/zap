...
WORK_DIR="$(mktemp -d)"
if [[ -n "${ZAP_CLI_BINARY:-}" && -x "$ZAP_CLI_BINARY" ]]; then
  TEST_ZAP_BINARY="$ZAP_CLI_BINARY"
elif [[ -x "$ROOT_DIR/native/target/release/zap" ]]; then
  TEST_ZAP_BINARY="$ROOT_DIR/native/target/release/zap"
elif [[ -x "$ROOT_DIR/bin/zap" ]]; then
  TEST_ZAP_BINARY="$ROOT_DIR/bin/zap"
elif [[ -x "$ROOT_DIR/bin/zap.exe" ]]; then
  TEST_ZAP_BINARY="$ROOT_DIR/bin/zap.exe"
else
  printf 'version validator regression: no built Zap binary found\n' >&2
  exit 2
fi
...
EXPECTED_VERSION="$CURRENT_VERSION" \
  ...
  ZAP_CLI_BINARY="$TEST_ZAP_BINARY" \
