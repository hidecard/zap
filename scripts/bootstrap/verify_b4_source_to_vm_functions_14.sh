...
let add = driver_seed_compile_source("fn add(a, b):\n    return a + b\nsay add(2, 3)", "add.zp")
let implicit = driver_seed_compile_source("fn implicit():\n    say 4\nsay implicit()", "implicit.zp")
let nested = driver_seed_compile_source("fn inner(value):\n    return value + 1\nfn outer(value):\n    return inner(value)\nsay outer(4)", "nested.zp")
let factorial = driver_seed_compile_source("fn factorial(n):\n    if n == 0:\n        return 1\n    return n * factorial(n - 1)\nsay factorial(5)", "factorial.zp")
let cross_frame = driver_seed_compile_source("fn raise_number():\n    raise 7\ntry:\n    say raise_number()\ncatch err:\n    say err\nsay 9", "cross_frame.zp")
let safe = driver_seed_compile_source("fn safe():\n    try:\n        raise 6\n    catch err:\n        return err\nsay safe()", "safe.zp")
let arity = driver_seed_compile_source("fn one(value):\n    return value\nsay one()", "arity.zp")
let outside = driver_seed_compile_source("return 1", "outside.zp")
let nested_function = driver_seed_compile_source("fn outer():\n    fn inner():\n        return 1\n    return 2", "nested_function.zp")
