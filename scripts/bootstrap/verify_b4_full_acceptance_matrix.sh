...
if "$PYTHON_BIN" -c "import sys; sys.path.insert(0, 'host/zap-bootstrap'); import c_backend; exit(0 if c_backend.find_c_compiler() else 1)" 2>/dev/null; then
  c_compiler_present=true
fi
if [[ "$c_compiler_present" == "true" ]]; then
  bash scripts/bootstrap/verify_b4_c_backend_acceptance.sh >/dev/null || fail "C backend acceptance gate failed"
else
  echo "INFO: skipping C backend acceptance gate (no system C compiler found)"
fi
run_zap() {
