...
for path in "$valid_fixture" "$valid_expected" "$compound_fixture" "$compound_expected" "$invalid_fixture" "$invalid_expected" "bootstrap/b1/parser.zp"; do
...
say parse_or_diagnostics(compound, "bootstrap/fixtures/parser/compound.zp")
...
cmp "$output" "$expected"
printf 'B1 Zap parser candidate differential passed: arithmetic AST, compound AST, and syntax rejection\n'
