...
cmp "$first" "$second"
[[ "$(wc -l < "$first")" -eq 6 ]] || { printf 'unexpected B2 candidate output line count\n' >&2; exit 1; }
...
printf 'B2 Zap type-checker candidate differential semantics passed: annotated, conditional, incompatible, function, function-call mismatch, collection-element mismatch\n'
