...
cmp "$first" "$second"
[[ "$(wc -l < "$first")" -eq 39 ]] || { printf 'unexpected B2 candidate output line count\n' >&2; exit 1; }
...
printf '%s\n' 'B2 Zap type-checker candidate differential semantics passed: exact collection-expression matrix included'
