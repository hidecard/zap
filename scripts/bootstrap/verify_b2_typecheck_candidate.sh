...
cmp "$first" "$second"
[[ "$(wc -l < "$first")" -eq 62 ]] || { printf 'unexpected B2 candidate output line count\n' >&2; exit 1; }
...
sed -n '62p' "$first" | jq -e '(.kind == "zap.typecheck") and (.ok == false) and (.schema_version == 1) and (.diagnostics[0].code == "ZAP-TYPE-001") and (.diagnostics[0].kind == "TypeError") and (.diagnostics[0].line == 3) and (.diagnostics[0].column == 1) and (.diagnostics[0].message == "unknown type annotation '\''NumberBox<number>'\''")' >/dev/null
