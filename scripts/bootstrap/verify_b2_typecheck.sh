...
set +e
generic_nested_option_list_incompatible=$(run_check generic_nested_option_list_incompatible 2>/tmp/zap-a3-generic-nested-option-list-incompatible-error)
...
grep -F "unexpected token after expression at 1:6" /tmp/zap-a3-generic-class-runtime-error >/dev/null || { printf 'generic class runtime diagnostic changed unexpectedly\n' >&2; cat /tmp/zap-a3-generic-class-runtime-error >&2; exit 1; }
printf 'A3 deferred class probe passed: generic class runtime rejection preserved\n'
set +e
generic_alias_deferred=$(run_check generic_alias_deferred 2>/tmp/zap-a3-generic-alias-error)
