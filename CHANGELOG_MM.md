...
- `scripts/bootstrap/assert_clean_repo_root.sh` ကို before/after diff နည်းလမ်းဖြင့် ပြန်ရေးထားသည်။ Run စတွင် `.zap-*` files များကို snapshot ဖမ်းပြီး primary+secondary subsets + forced-failure path ကို run ကာ AFTER snapshot တွင် ပေါ်လာသော်လည်း BEFORE snapshot တွင် မပါသော files များကိုသာ flag လုပ်သည်။ `zap lsp` သည် ကိုယ်ပိုင် scratch files များကို workspace root ุင် ဖန်တီးနေသော developer environment များတွင် gate ကို correct ဖြစ်စေသည်။ Mid-run planted-file negative test ဖြင့် gate သည် real leaks ကို detect လုပ်ပြီး nonzero ဖြင့် ထွက်ကြောင်း အတည်ပြုထားသည်။
- P0 release-hygiene gate အသစ် နှစ်ခု (`scripts/bootstrap/assert_clean_repo_root.sh` နှင့် `scripts/bootstrap/run_zap_refactor_smoke.sh`) ကို `.github/workflows/ci.yml` ထဲသို ချိတ်ဆက်ထားသည်။ push နှင့် PR တိုင်းတွင် `scripts/test_validate_release_version.sh` နှင့်အတူ run လုပ်သည်။
...
### Release engineering
- Exact {"score": 7} map-literal annotation evidence ထည့်ပြီး production networking behavior မပြောင်းဘဲ native macOS HTTP response test harness ကို harden လုပ်သည်။
