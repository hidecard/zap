...
- `scripts/bootstrap/assert_clean_repo_root.sh` ကို before/after diff နည်းလမ်းဖြင့် ပြန်ရေးထားသည်။ Run စတွင် `.zap-*` files များကို snapshot ဖမ်းပြီး primary+secondary subsets + forced-failure path ကို run ကာ AFTER snapshot တွင် ပေါ်လာသော်လည်း BEFORE snapshot တွင် မပါသော files များကိုသာ flag လုပ်သည်။ `zap lsp` သည် ကိုယ်ပိုင် scratch files များကို workspace root တွင် ဖန်တီးနေသော developer environment များတွင် gate ကို correct ဖြစ်စေသည်။ Mid-run planted-file negative test ဖြင့် gate သည် real leaks ကို detect လုပ်ပြီး nonzero ဖြင့် ထွက်ကြောင်း အတည်ပြုထားသည်။
