...
Zap v2.1.4 is the first protected v2.1 release prepared with the v2.1-E release-engineering pipeline. It includes the production async/runtime, tooling, registry, bounded I/O, deterministic artifact, signing, provenance, and post-publish verification work completed for this release line.
...
Download the release assets together with `zap-2.1.4-release-signing-key.asc`, import the public key into an isolated verification keyring, and run:

```bash
GNUPGHOME=/secure/verification/gnupg \
  bash scripts/verify_published_release.sh 2.1.4 ./published-release
```
...
| Target | Artifact |
|---|---|
| Linux x86_64 | `zap-2.1.4-linux-x86_64.tar.gz` |
| macOS ARM64 | `zap-2.1.4-macos-arm64.tar.gz` |
| Windows x86_64 | `zap-2.1.4-windows-x86_64.zip` |

## Release assets checklist

- [ ] All three platform archives are attached.
- [ ] Per-artifact `.sha256` sidecars are attached.
- [ ] `zap-2.1.4-checksums.sha256` is attached.
- [ ] `zap-2.1.4-manifest.json` is attached.
- [ ] `zap-2.1.4-provenance.json` is attached.
- [ ] Detached `.asc` signatures are attached for every archive and metadata file.
- [ ] `zap-2.1.4-release-signing-key.asc` is attached.
...
- English release notes: `docs/RELEASE_2.1.4_EN.md`
- Burmese release notes: `docs/RELEASE_2.1.4_MM.md`
