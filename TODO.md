...
- **Completed in the command-dispatch slice:** `driver_command()` now provides one deterministic Zap-owned entry point for `check`, `build`, `run`, and `test`, and returns stable `ZAP-DRIVER-001` diagnostics for unsupported commands; native CLI delegation remains a separate pending boundary.
- **Typed-IR ownership promotion slice in progress:** `driver_typed_ir_ownership()` now promotes only schema-4, checker-inferred, diagnostic-free, shape-valid, deterministic typed-IR records to `ownership=zap_owned` with `reference_owner=zap`; older or incomplete records remain `ownership=candidate` and fail closed.
