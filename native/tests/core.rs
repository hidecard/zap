...
#[test]
fn lock_migrate_is_idempotent_for_current_lockfile() {
    let root = std::env::temp_dir().join("zap_lock_migrate_current_project");
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    std::fs::write(
        root.join("zap.toml"),
    ...
    .unwrap();
    std::fs::write(root.join("main.zp"), "say \"migration\"\n").unwrap();
    let generated = Command::new(binary())
        .args(["lock", root.to_str().unwrap()])
        .output()
        .unwrap();
    ...
    let current = std::fs::read_to_string(root.join("zap.lock")).unwrap();
    std::fs::write(
        root.join("zap.lock"),
        current.replacen("lockfile_version = 1", "lockfile_version = 2", 1),
    )
    .unwrap();
    let migrated = Command::new(binary())
        .args(["lock-migrate", root.to_str().unwrap()])
        .output()
        .unwrap();
    let _ = std::fs::remove_dir_all(&root);
    assert!(
    ...
    assert_eq!(
        ...
        "zap.lock is already at lockfile version 2\n"
    );
}

#[test]
fn lock_migrate_requires_registry_index_for_legacy_registry_lockfile() {
    let root = std::env::temp_dir().join("zap_lock_migrate_legacy_project");
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    std::fs::write(
        root.join("zap.toml"),
    ...
    .unwrap();
    std::fs::write(root.join("main.zp"), "say \"legacy\"\n").unwrap();
    std::fs::write(
        root.join("zap.lock"),
        "# This file is generated by Zap. Do not edit manually.\nlockfile_version = 1\n\n[package]\nname = \"legacy-demo\"\nversion = \"0.1.0\"\n\n[dependencies]\ncore = \"1.0\"\n",
    )
    ...
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr)
        .contains("legacy registry lockfile requires migration"));
}

#[test]
fn lock_migrate_is_listed_in_help() {
    let output = Command::new(binary()).arg("--help").output().unwrap();
    assert!(output.status.success());
