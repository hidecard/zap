...
### Release engineering
- Add bounded direct some(1) option-constructor annotation evidence.
...
- Added a fail-closed native CLI regression harness for malformed generic, unknown-annotation, and incompatible-annotation sources. Each case must fail nonzero without panic or unchecked-unwrap signatures; CI and release preflight now require the gate. Bootstrap remains B0 and Rust remains the reference owner.
...
- Replaced the parser candidate’s substring-based bracket check with token-driven delimiter diagnostics covering missing and unexpected closing brackets.
...
### Release engineering
- Reject duplicate method/path Web route registrations during zap web check and zap web routes, with shared validation and bilingual documentation.
...
### Release engineering
- Add bounded incremental LSP synchronization, UTF-aware range validation, sequential edit limits, and regression evidence while preserving explicit cross-file refactoring boundaries.
