...
- Extended `scripts/bootstrap/assert_clean_repo_root.sh` with a secondary subset that exercises B1/B3/B4 verifiers refactored to use the `run_zap()` helper, so the in-flight portability refactor stays cleanup-safe end-to-end before commit.
- Added `scripts/bootstrap/run_zap_refactor_smoke.sh` smoke gate that runs the full 150-script refactor with three phases (bash -n parse check, end-to-end smoke run with 60s timeout per script, before/after repo-root diff to distinguish script-induced leaks from pre-existing LSP-created artifacts). Wired into `make test` as `bootstrap-refactor-smoke-test`. The gate discovered that `zap lsp` was actively creating `.zap-*.zp` files in the workspace root — a fact that explained previously-anomalous `assert_clean_repo_root.sh` reports and led to the before/after snapshot design.
- Rewrote `scripts/bootstrap/assert_clean_repo_root.sh` to use the same before/after repo-root diff approach: snapshots the `.zap-*` file set at the start of the run, runs the primary+secondary subsets + a forced-failure path, and only flags files that appear in the AFTER snapshot but not in the BEFORE snapshot. This makes the gate correct in developer environments where `zap lsp` is creating its own scratch files in the workspace root. Negative test (mid-run planted file) confirmed the gate still detects real leaks and exits nonzero.
- Wired the two new P0 release-hygiene gates (`scripts/bootstrap/assert_clean_repo_root.sh` and `scripts/bootstrap/run_zap_refactor_smoke.sh`) into `.github/workflows/ci.yml` so they run on every push and PR alongside `scripts/test_validate_release_version.sh`. Both gates depend on `bin/zap` already being built, which the existing `cargo build --release` step provides.
