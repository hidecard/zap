...
Zap ကို စတင်လေ့လာသူများအတွက် ရိုးရှင်းပြီး နားလည်ရလွယ်ကူစေရန် ရည်ရွယ်ထားပါသည်။ Indentation-based blocks၊ variables၊ functions၊ collections၊ conditions၊ loops၊ classes၊ modules၊ type annotations နှင့် Result/Option values များကို language core ထဲတွင် ထည့်သွင်းထားပါသည်။ Native source run တစ်ကြိမ်စီတွင် module cache၊ import cycle၊ execution depth၊ workspace confinement၊ logical budget၊ object store နှင့် parent-linked lexical closure frame များ မရောနှောစေရန် explicit `ExecutionContext` တစ်ခုကို အသုံးပြုပါသည်။ LSP server သည် open document များကို per-session `LspState` ထဲတွင် ပိုင်ဆိုင်ပါသည်။ Normal source program နှင့် local module များသည် canonical AST boundary မှတစ်ဆင့်သာ execute လုပ်ပြီး line interpreter ကို older line-bodied function record များအတွက် compatibility boundary အဖြစ်သာ ထားရှိပါသည်။

## Project Status

Zap သည် production-ready language ecosystem တစ်ခုအဖြစ် တိုးတက်နေပါသည်။ Native Rust runtime၊ direct AST execution၊ structured diagnostics၊ `ZAP-MEMORY-001` stable memory-limit diagnostic၊ control-flow type narrowing၊ module visibility၊ OOP rules၊ deterministic dependency lockfiles၊ registry resolution၊ checksum verification၊ offline package reuse၊ executor-backed context-owned language scheduling၊ `ScheduledFuture` handle၊ cooperative `task_cancel`၊ poll-budget `task_join_timeout` ပါသော async runtime နှင့် stdio LSP/editor integration များကို ထည့်သွင်းထားပါသည်။ M2-VERIFY-01 တွင် fixed-seed bounded replay၊ repeated semantic outcome digest နှင့် CI/release-preflight evidence များကို ထည့်သွင်းထားပါသည်။

| အချက် | လက်ရှိအခြေအနေ |
|---|---|
| လက်ရှိ release line | `v2.1.14` |
...
| Verification status | M2-VERIFY-01 bounded 12-round replay နှင့် M2-VERIFY-02 native Linux/Windows/macOS async/archive matrix evidence |
...
Zap သည် language core ကို သေးငယ်၊ ရှင်းလင်းပြီး လေ့လာရလွယ်ကူအောင် တည်ဆောက်ထားပါသည်။ `.zp` file များကို native executable ဖြင့် တိုက်ရိုက် run နိုင်ပြီး parser ပိုင် source များကို canonical AST ဖြင့် execute လုပ်ပါသည်။ Async task များအတွက် context-owned `ScheduledFuture`၊ `task_join`၊ `task_is_ready`၊ cooperative `task_cancel` နှင့် poll-budget `task_join_timeout` များကို အသုံးပြုနိုင်ပါသည်။ Fixed-seed bounded replay verification သည် malformed input နှင့် repeated semantic outcome များကို CI တွင် ထပ်မံစစ်ဆေးနိုင်စေပါသည်။ Per-run workspace/module/execution state နှင့် logical budget/object counter များကို stable `memory_stats()` lifecycle fields၊ first-class callable value များနှင့် parent-linked `EnvFrame` closure များအတူ explicit runtime context ဖြင့် ခွဲခြားထားပြီး LSP document state ကို per-session `LspState` ဖြင့် ပိုင်ဆိုင်ထားပါသည်။ Legacy line execution ကို compatibility-only boundary အဖြစ် ကန့်သတ်ထားပါသည်။ နောင်တွင် web၊ AI၊ mobile နှင့် IoT libraries များ တည်ဆောက်ရန် foundation အဖြစ် အသုံးပြုနိုင်ပါသည်။

## Installation

Zap သည် သီးခြား language runtime မလိုအပ်သော standalone native executable အဖြစ် ဖြန့်ချိပါသည်။ v2.1.14 အတွက် မိမိအသုံးပြုသည့် operating system နှင့် CPU architecture ကိုက်ညီသော archive ကို [v2.1.14 GitHub Release](https://github.com/hidecard/zap/releases/tag/v2.1.14) မှ download လုပ်ပြီး checksum ကို verify လုပ်ကာ extract လုပ်ပါ။

v2.1.14 official archive များမှာ `zap-2.1.14-linux-x86_64.tar.gz`၊ `zap-2.1.14-macos-arm64.tar.gz` နှင့် `zap-2.1.14-windows-x86_64.zip` ဖြစ်ပါသည်။
