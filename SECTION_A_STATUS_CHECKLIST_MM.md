...
- [ ] **Zap-owned VM execution** — Zap-written VM သည် immutable local environment အပေါ် `load`/`store`၊ arbitrary stack-depth pop၊ empty-stack `stack_underflow` result၊ VM-state error propagation၊ arithmetic၊ comparison၊ logical နှင့် unary `not` opcodes၊ deterministic `call` (`identity`/`abs`)၊ `jump` နှင့် `jump_if_false` semantics များကို B3 gates ဖြင့် pass ထားပြီး Typed-IR → canonical bytecode → VM mutation loop၊ comparison branch၊ logical condition၊ call နှင့် failure paths ကို expanded verifier ဖြင့် စစ်ဆေးထားသော်လည်း full bytecode instruction set၊ arbitrary compiler-generated bytecode နှင့် native VM ownership transfer မပြီးသေးပါ။
