...
- [ ] **Arbitrary-program parser coverage** — mixed top-level sequence အတွင်း block header တွေ့ပါက indentation parser သို့ route ပြောင်းခြင်း၊ typed/untyped declaration parsing၊ decimal-preserving numeric tokenization၊ `not` tokenization၊ generic function parameter/body route၊ generic class member/method route၊ declaration-route ၆-case verifier၊ nested assignment/block span parity နှင့် native-compatible positional/named (`name = value`) call-argument AST route ကို တိုးချဲ့ပြီး B1 lexer/parser-diagnostic regressions နှင့် native differential matrix ၁၈ fixtures pass ထားသော်လည်း all grammar forms နှင့် arbitrary mixed top-level sequences အတွက် full parser ownership မပြီးသေးပါ။
...
- [ ] **General typed-IR production** — declaration/list/map emission၊ list/index disambiguation၊ generic type-alias node၊ generic/function metadata၊ arithmetic/comparison/logical binary expression၊ unary `not`၊ short-circuit `and/or` jump sequence၊ positional/named multi-argument calls၊ index/member/await/propagate expression nodes၊ multiline mixed-program route၊ return/say/raise/if/while/for payload metadata၊ mutable assignment store၊ loop `break`/`continue` placeholders နှင့် recursive call/control-flow lowering foundation ကို တိုးချဲ့ပြီး 100+ generic/type assertions၊ expression regressions၊ named-argument ၆ cases၊ lexer/parser/typecheck/typed-IR artifact assembly ၁၂ cases နှင့် expanded Typed-IR → canonical bytecode → Zap VM mutation/branch gate pass ထားသော်လည်း arbitrary blocks၊ nested loop context propagation နှင့် complete bytecode integration မပြီးသေးပါ။
...
- [ ] **Zap-owned package/build pipeline** — Zap-written manifest validation၊ lock dependency matching၊ duplicate rejection၊ exact lock coverage၊ required artifact-kind/package/target validation၊ offline contract နှင့် deterministic serialized manifest foundation များကို တိုးချဲ့ပြီး B3/B4 regressions pass ထားသော်လည်း actual compiler artifact production နှင့် native Rust ownership transfer မပြီးသေးပါ။

- [ ] **Zap-owned VM execution** — Zap-written VM သည် immutable local environment အပေါ် `load`/`store`၊ arbitrary stack-depth pop၊ empty-stack `stack_underflow` result၊ VM-state error propagation၊ arithmetic၊ comparison၊ logical၊ unary `not` နှင့် short-circuit `jump_if_true`/`jump_if_false` opcodes၊ deterministic `call` (`identity`/`abs`)၊ `jump`၊ `dup`၊ `pop` semantics များကို B3 gates ဖြင့် pass ထားပြီး Typed-IR → canonical bytecode → VM mutation loop၊ comparison branch၊ logical short-circuit RHS skip၊ call၊ `break`/`continue` နှင့် failure paths ကို expanded verifier ဖြင့် စစ်ဆေးထားသော်လည်း full bytecode instruction set၊ arbitrary compiler-generated bytecode နှင့် native VM ownership transfer မပြီးသေးပါ။

- [ ] **Platform-seed acceptance** — Zap-written runner contract သည် clock/console/file/memory ကို allow၊ network/process ကို deny ပြီး stage ownership၊ structural validity၊ deterministic produced-artifact handoff၊ input-kind checked stage execution၊ multi-stage plan execution/digest-count rejection နှင့် source→lexer→parser→typecheck→typed-IR→bytecode→VM stage chain ကို စစ်ဆေးကာ runner ၁၅ နှင့် seed-pipeline ၁၂ cases pass ထားသော်လည်း status သည် `contract_only` ဖြစ်ပြီး bootstrap output ကို platform seed မှ self-build/run လုပ်နိုင်ကြောင်း မပြနိုင်သေးပါ။

- [ ] **Full B4 self-hosting** — မရသေးပါ။ Runner contract၊ source-to-VM seed-pipeline ၁၂ cases၊ bounded source-to-bytecode-to-VM ၁၀ cases၊ bounded source self-rebuild/identical VM output evidence၊ canonical rebuild-manifest byte comparison ၆ cases နှင့် compiler-artifact two-pass equality/mismatch verifier ၈ cases pass ထားသော်လည်း compiler support သည် bounded slice သာဖြစ်ပြီး comparison သည် bounded candidate artifacts အပေါ်သာဖြစ်ကာ `native_independent=false` နှင့် `contract_only` အခြေအနေကို verifier က အတည်ပြုထားဆဲဖြစ်ကာ actual arbitrary Zap compiler self-rebuild မရှိသေးပါ။
...
1. **Verification of the Zap-owned pipeline** — `verify_b4_owned_pipeline_42.sh` နှင့် `verify_b4_source_to_vm_*.sh` တို့ဖြင့် `driver_execute_owned_pipeline` (source → typed-IR → bytecode → VM) ကို toolchain ရရှိမှသာ run ပြီး confirm လုပ်ရန်။
