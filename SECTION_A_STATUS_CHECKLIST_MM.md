...
- [ ] **General typed-IR production** — declaration/list/map emission၊ list/index disambiguation၊ generic type-alias node၊ generic/function metadata၊ arithmetic/comparison/logical binary expression၊ positional/named multi-argument calls၊ index/member/await/propagate expression nodes၊ multiline mixed-program route နှင့် return/say/raise/if/while/for payload metadata ကို တိုးချဲ့ပြီး 100+ generic/type assertions၊ expression regressions၊ named-argument ၆ cases၊ lexer/parser/typecheck/typed-IR artifact assembly ၁၂ cases နှင့် Typed-IR → canonical bytecode → Zap VM arithmetic/say/rejection gate pass ထားသော်လည်း arbitrary blocks နှင့် complete bytecode integration မပြီးသေးပါ။
...
- [ ] **Zap-owned VM execution** — Zap-written VM သည် arbitrary stack-depth pop၊ empty-stack `stack_underflow` result၊ VM-state error propagation နှင့် add/subtract/multiply/divide arithmetic opcodes များကို B3 ownership/VM platform gates ဖြင့် pass ထားပြီး Typed-IR → canonical bytecode → VM arithmetic/say handoff နှင့် unsupported-node rejection gate အသစ် pass ထားသော်လည်း full bytecode instruction set၊ arbitrary compiler-generated bytecode နှင့် native VM ownership transfer မပြီးသေးပါ။
