...
- [ ] **General typed-IR production** — declaration/list/map emission၊ list/index disambiguation၊ generic type-alias node၊ generic/function metadata၊ arithmetic/comparison/logical binary expression၊ positional/named multi-argument calls၊ index/member/await/propagate expression nodes၊ multiline mixed-program route၊ return/say/raise/if/while/for payload metadata နှင့် recursive call/control-flow lowering foundation ကို တိုးချဲ့ပြီး 100+ generic/type assertions၊ expression regressions၊ named-argument ၆ cases၊ lexer/parser/typecheck/typed-IR artifact assembly ၁၂ cases နှင့် expanded Typed-IR → canonical bytecode → Zap VM ၁၂-case gate pass ထားသော်လည်း arbitrary blocks နှင့် complete bytecode integration မပြီးသေးပါ။
...
- [ ] **Zap-owned VM execution** — Zap-written VM သည် arbitrary stack-depth pop၊ empty-stack `stack_underflow` result၊ VM-state error propagation၊ add/subtract/multiply/divide arithmetic၊ deterministic `call` (`identity`/`abs`)၊ `jump` နှင့် `jump_if_false` semantics များကို B3 ownership/VM platform gates ဖြင့် pass ထားပြီး Typed-IR → canonical bytecode → VM arithmetic/say/call/if/while handoff နှင့် unknown-call failure path ကို expanded verifier ဖြင့် စစ်ဆေးထားသော်လည်း mutable-variable loop lowering၊ full bytecode instruction set၊ arbitrary compiler-generated bytecode နှင့် native VM ownership transfer မပြီးသေးပါ။
