...
Download the archive matching your operating system and CPU architecture from the [v2.11.17 release page](https://github.com/hidecard/zap/releases/tag/v2.11.17), verify its checksum and signature, extract it, and place the `zap` executable on `PATH`.
...
```bash
tar -xzf zap-2.11.17-macos-arm64.tar.gz
cd zap
chmod +x install.sh
./install.sh
zap --version
...
The expected archive is `zap-2.11.17-windows-x86_64.zip`.
