...
| Current status | [Canonical current-status page](docs/CURRENT_STATUS_EN.md) |

## Install

Download the archive matching your operating system and CPU architecture from the [v2.11.7 release page](https://github.com/hidecard/zap/releases/tag/v2.11.7), verify its checksum and signature, extract it, and place the `zap` executable on `PATH`.

### Linux

```bash
tar -xzf zap-2.11.7-linux-x86_64.tar.gz
cd zap
bash install.sh
zap --version
```
...
```bash
tar -xzf zap-2.11.7-macos-arm64.tar.gz
cd zap
chmod +x install.sh
./install.sh
zap --version
...
The expected archive is `zap-2.11.7-windows-x86_64.zip`.
...
Zap remains at **B0**. The Zap lexer/parser/type-checker/typed-IR work currently documented under `bootstrap/` is **provisional and corpus-limited**: it provides differential evidence for selected fixtures, while the Rust native implementation remains the reference owner. The v2.11.7 function fixtures cover one annotated function, return propagation, a compatible numeric call, and a stable incompatible-call diagnostic; they do not establish a general self-hosted compiler.
...
The current source baseline is v2.11.7. The [canonical current-status page](docs/CURRENT_STATUS_EN.md) records the active B0 boundary and the signed provenance fields for the latest published release. The preceding v2.3.0, v2.2.7, and earlier release records remain available in [GitHub Releases](https://github.com/hidecard/zap/releases) and the bilingual `CHANGELOG` files. Release artifacts are published only after version consistency, native tests, cross-platform builds, security checks, documentation checks, and installer verification pass.
