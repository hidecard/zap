...
Zap is distributed as a standalone native executable. No separate language runtime is required. Download the archive that matches your operating system and CPU architecture from the [v2.1.9 GitHub Release](https://github.com/hidecard/zap/releases/tag/v2.1.9), verify the checksum when available, extract it, and make the `zap` executable available on your `PATH`.
...
For the current v2.1.9 release, the platform assets are `zap-2.1.9-linux-x86_64.tar.gz`, `zap-2.1.9-macos-arm64.tar.gz`, and `zap-2.1.9-windows-x86_64.zip`. The exact archive filename may change with each release. Select the asset whose platform and architecture match your computer; do not install a Linux archive on Windows or a macOS archive on Linux.
...
```bash
tar -xzf zap-2.1.9-linux-x86_64.tar.gz
cd zap
bash install.sh
```
...
1. Download the macOS ARM64 `.tar.gz` archive from the [v2.1.9 release](https://github.com/hidecard/zap/releases/tag/v2.1.9).
2. Extract it and enter the extracted directory:

```bash
tar -xzf zap-2.1.9-macos-arm64.tar.gz
cd zap
```
...
1. Download the Windows x86_64 `.zip` archive from the [v2.1.9 release](https://github.com/hidecard/zap/releases/tag/v2.1.9).
