...
Download the archive matching your operating system and CPU architecture from the [v2.11.8 release page](https://github.com/hidecard/zap/releases/tag/v2.11.8), verify its checksum and signature, extract it, and place the `zap` executable on `PATH`.

### Linux

```bash
tar -xzf zap-2.11.8-linux-x86_64.tar.gz
cd zap
bash install.sh
zap --version
```
...
```bash
tar -xzf zap-2.11.8-macos-arm64.tar.gz
cd zap
chmod +x install.sh
./install.sh
zap --version
...
The expected archive is `zap-2.11.8-windows-x86_64.zip`.
