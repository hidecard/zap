...
| P0-05 | Deterministic နှင့် production async boundary | Partial — M2-ASYNC-01 scheduling ပြီးစီး | Deterministic executor၊ context-owned `ScheduledFuture` language handle များ၊ fixed-worker၊ bounded network/process adapter များ၊ cancellation behavior၊ descriptive `async_capabilities()` report၊ typed resource-limit preflight validation၊ TCP request-size admission check နှင့် target-named CI artifact များပါသော reproducible Linux/Windows/macOS focused matrix ကို document/expose လုပ်ပြီးဖြစ်သည်။ Language-level cancellation/timeout control၊ full reactor semantics နှင့် tooling synchronization တို့သာ ကျန်ရှိသည်။ |
