...
The post-v2.2.5 review confirmed four correctness/security-boundary defects and one bounded-operation hardening opportunity. The maintenance working tree fixes `read_lines` and `write_lines` so both canonical AST and retained legacy execution route through workspace confinement; makes `build --locked` require an existing `zap.lock` even for dependency-free projects; rejects malformed and out-of-range URL ports; makes test discovery ignore symlink entries and visit canonical directories only once; and bounds `sleep` and `pow` with explicit stable limits while using checked exponentiation-by-squaring.

v2.2.5 နောက်ပိုင်း review တွင် correctness/security boundary ဆိုင်ရာ defect လေးခုနှင့် bounded-operation hardening တစ်ခုကို အတည်ပြုခဲ့ပါသည်။ Maintenance working tree တွင် canonical AST နှင့် retained legacy execution နှစ်မျိုးလုံး၏ `read_lines`/`write_lines` ကို workspace confinement ဖြင့် ကာကွယ်ထားပါသည်။ Dependency မရှိသော project များအတွက်ပင် `build --locked` သည် ရှိပြီးသား `zap.lock` ကို မဖြစ်မနေလိုအပ်စေပါသည်။ Malformed နှင့် range ကျော် URL port များကို reject လုပ်ပြီး test discovery သည် symlink entry များကို skip လုပ်ကာ canonical directory တစ်ခုကို တစ်ကြိမ်သာ visit လုပ်ပါသည်။ `sleep` နှင့် `pow` တွင် explicit stable limit များ ထည့်ပြီး checked exponentiation-by-squaring ကို အသုံးပြုထားပါသည်။
...
The following items are intentionally not implemented in this maintenance release. Process-group descendant cleanup, race-resistant descriptor-relative filesystem APIs, DNS-to-connection pinning, AST/typed-IR replacement of the heuristic checker, syntax-aware formatting, per-test timeout/coverage/profiling, and dependency-version upgrades require separate design or lockfile work. The current RustSec scan found six advisories in the locked graph involving `idna 0.4.0`, `rustls-webpki 0.102.8`, and dev-only `time 0.3.30`; those upgrades are recorded for a separate dependency-maintenance pass because the repository release policy explicitly forbids `cargo update` and lockfile regeneration in the focused release work.

အောက်ပါအရာများကို ဤ maintenance release တွင် ရည်ရွယ်ချက်ရှိရှိ မထည့်သွင်းပါ။ Process-group descendant cleanup၊ race-resistant descriptor-relative filesystem API၊ DNS-to-connection pinning၊ heuristic checker ကို AST/typed-IR checker အဖြစ် ပြောင်းလဲခြင်း၊ syntax-aware formatting၊ per-test timeout/coverage/profiling နှင့် dependency-version upgrade များသည် သီးခြား design သို့မဟုတ် lockfile work လိုအပ်ပါသည်။ လက်ရှိ RustSec scan တွင် locked graph အတွင်း `idna 0.4.0`၊ `rustls-webpki 0.102.8` နှင့် dev-only `time 0.3.30` ဆိုင်ရာ advisory ခြောက်ခု တွေ့ရှိခဲ့ပါသည်။ Focused release policy က `cargo update` နှင့် lockfile regeneration ကို တားမြစ်ထားသဖြင့် ထို dependency upgrades များကို သီးခြား dependency-maintenance pass အဖြစ် မှတ်တမ်းတင်ထားပါသည်။
...
The next core-maintenance pass should first plan the locked dependency upgrades and a modern advisory gate, then address process-group policy per supported operating system and the residual filesystem race boundary. Only after those contracts are specified should the project consider deeper test-runner, formatter/linter, typed-IR, or incremental LSP work. Traits/interfaces/composition, web/mobile/IoT hosts, and other framework ecosystem proposals remain future branch work rather than current release tasks.

ဤ maintenance release နောက်ပိုင်းတွင် ပထမဆုံး locked dependency upgrade နှင့် modern advisory gate ကို စီစဉ်သင့်ပြီး၊ ထို့နောက် support လုပ်သော operating system တစ်ခုချင်းစီအတွက် process-group policy နှင့် ကျန်ရှိသော filesystem race boundary ကို ဖြေရှင်းသင့်ပါသည်။ ထို contracts များ သတ်မှတ်ပြီးမှသာ test-runner၊ formatter/linter၊ typed-IR သို့မဟုတ် incremental LSP အလုပ်များကို ဆက်လက်စဉ်းစားသင့်ပါသည်။ Traits/interfaces/composition၊ web/mobile/IoT host များနှင့် အခြား framework ecosystem proposal များသည် current release task မဟုတ်ဘဲ future separate branch work ဖြစ်ပါသည်။
...
[7]: https://github.com/hidecard/zap/releases/tag/v2.2.5 "Zap v2.2.5 release"
