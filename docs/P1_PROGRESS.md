...
| P1-05 conformance/property/security layer runner | Implemented (initial) | `scripts/test_p105_layers.sh` runs deterministic parser, lexer, JSON, lockfile, registry-security, collection/filesystem, cancellation, async-scheduler, and seven-case fuzz-style CLI mutation regressions; all malformed mutations reject safely without panic. CI also retains Linux, Windows, and macOS build/test matrix coverage. Long-running fuzz targets, allocator/heap-level counters, and additional platform-specific input cases remain. |

## Current verification baseline

The native Rust suite currently passes **397 tests**: 143 unit tests and 254 integration tests. The P1-05 deterministic layer runner passes its parser/lexer/JSON/lockfile/security corpus, collection regression, and async cancellation/scheduler cases. The repository includes the runnable example [`examples/default_parameters.zp`](../examples/default_parameters.zp), synchronized English/Burmese type-narrowing, package, standard-library, benchmark, and release guides, and README links for the current learning materials. The native CLI version output matches the `v1.0.0` release line. `cargo fmt --check` and `git diff --check` pass. Stable Rust Clippy remains enforced by the GitHub Actions release workflow; it is not claimed as locally verified when the local sandbox lacks the Clippy component.
