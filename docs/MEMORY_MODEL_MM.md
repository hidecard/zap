...
Native test `parent_linked_closures_preserve_mutation_after_outer_return` နှင့် `live_closures_share_reassigned_outer_cells_without_breaking_shadowing_or_recursion` တို့သည် live binding-cell capture ကို စစ်ဆေးသည်။ Native test `cyclic_object_graph_can_be_explicitly_broken` သည် self-referential object တစ်ခု ဖန်တီးပြီး cycle ရှိကြောင်း စစ်ဆေးသည်။ ထို့နောက် field များကို ရှင်းလင်းကာ field allocation ပြန်လွှတ်နိုင်ကြောင်း စစ်ဆေးသည်။ `conflicting_object_borrows_return_typed_failures` regression သည် mutable field borrow တစ်ခုကို active ထားပြီး ပြိုင်တူ read/write များအတွက် panic မဖြစ်ဘဲ deterministic `BorrowError` ပြန်လာကြောင်း စစ်ဆေးသည်။ M2-MEM-02 regression များသည် nested logical sizing၊ callable capture validation၊ finalized object charge၊ failed byte/output rollback၊ context တစ်ခုချင်းစီ၏ validation/cleanup counter၊ output/task admission နှင့် ယခင် run က ထိန်းထားသည့် object များမှ reset ပြီးနောက် state မရောနှောမှုကို စစ်ဆေးသည်။ ဤ test များသည် memory contract ကို အတည်ပြုခြင်းဖြစ်ပြီး arbitrary cycle များကို အလိုအလျောက် garbage collect လုပ်သည်ဟု ဆိုလိုခြင်းမဟုတ်ပါ။
