...
Zap v2.2.4 တွင် recoverable value အဖြစ် `ok(value)`၊ `err(value)`၊ `some(value)` နှင့် `option_none()` ကို အသုံးပြုနိုင်သည်။ `is_ok`၊ `is_err`၊ `is_some` နှင့် `is_option_none` ဖြင့် value အမျိုးအစားကို စစ်ဆေးနိုင်ပြီး `unwrap` သို့မဟုတ် `unwrap_or` ဖြင့် value ကို ရယူနိုင်သည်။
...
လက်ရှိ roadmap နှင့် release details ကို [`DOCUMENTATION_NAVIGATION_MM.md`](DOCUMENTATION_NAVIGATION_MM.md)၊ [`TYPECHECK_CONFORMANCE_MATRIX_MM.md`](TYPECHECK_CONFORMANCE_MATRIX_MM.md)၊ [`NEXT_TODO_PLAN_MM.md`](NEXT_TODO_PLAN_MM.md) နှင့် [`RELEASE_2.2.4_MM.md`](RELEASE_2.2.4_MM.md) တွင် ဖတ်ရှုပါ။
