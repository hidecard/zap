...
Zap v2.1.5 သည် ပြီးစီးထားသော v2.1-D runtime/tooling အလုပ်များနှင့် v2.1-E release-engineering pipeline ကို အခြေခံထားသည့် protected v2.1 patch release ဖြစ်ပါသည်။ ဤ release တွင် production-oriented asynchronous execution၊ bounded I/O adapters၊ registry service controls၊ synchronized developer tooling၊ reproducible artifacts၊ signed metadata၊ provenance နှင့် post-publish verification တို့ ပါဝင်ပါသည်။
...
Release pipeline တွင် dry-run-first version/changelog bump helper၊ tag-gated release preflight၊ deterministic three-target artifact aggregation၊ per-artifact နှင့် aggregate SHA-256 verification၊ `zap.release-manifest.v1`၊ `zap.provenance.v1`၊ detached GPG signatures၊ `zap-2.1.5-release-signing-key.asc` public signing-key asset နှင့် post-publish release verification တို့ ပါဝင်ပါသည်။ Unix archive များအတွက် repository-owned platform-neutral deterministic tar.gz helper ကို အသုံးပြုပြီး Windows ZIP entry များအတွက် ZIP က ထောက်ပံ့သော အနိမ့်ဆုံး timestamp နှင့် မှန်ကန်သော top-level `zap/` directory ကို အသုံးပြုသဖြင့်၊ clean-profile installer verification အတွင်း PowerShell read-only `$HOME` variable collision ကိုလည်း ရှောင်ရှားထားပြီး reproducible packaging နှင့် release verification ကို တစ်သမတ်တည်း ထိန်းသိမ်းထားပါသည်။ Release incident များအတွက် bilingual rollback/quarantine runbook ကိုလည်း ထည့်သွင်းထားပါသည်။

## Supported Targets

| Platform | Target | Artifact |
|---|---|---|
| Linux | x86_64 GNU | `zap-2.1.5-linux-x86_64.tar.gz` |
| macOS | ARM64 | `zap-2.1.5-macos-arm64.tar.gz` |
| Windows | x86_64 MSVC | `zap-2.1.5-windows-x86_64.zip` |
...
### v2.1.5 signing နှင့် publication hardening
- Dedicated signed-release publication path နှင့် platform အစုံ reproducible archive verification ကို ထိန်းသိမ်းထားပါသည်။
