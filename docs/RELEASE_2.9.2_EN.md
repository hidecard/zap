...
Zap v2.9.2 is a maintenance release that fixes the first-run developer experience and tightens production-facing defaults. Fresh Web projects now use the requested project name, generate linter-compatible source, and pass the documented validation workflow.

The Unix installer and portable uninstaller now persists a configured `ZAP_INSTALL_DIR` instead of always writing the default `~/.local/bin` path. The Makefile now points to the actual legacy test suite and exposes locked native, host, legacy, and aggregate test targets.
