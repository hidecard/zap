...
**Status:** Design gate; A3 is not complete and Zap remains B4 candidate. Rust remains the reference compiler, type checker, runtime, and diagnostic owner.
...
The current provisional checkpoint adds three Rust-reference-backed malformed-header fixtures: `generic_empty_params.zp` rejects `fn empty<>` with `generic type-parameter list cannot be empty`; `generic_duplicate_params.zp` rejects a repeated `T` with `duplicate generic type parameter: T`; and `generic_invalid_param.zp` rejects lowercase `t` with `invalid generic type parameter 't'`. Each produces the stable `ZAP-SYNTAX-001` / `SyntaxError` diagnostic at line 1, column 1, and the candidate differential verifier reproduces these exact corpus cases. The same checkpoint now includes `generic_list_wrapper.zp`, where `list_keep<T>(list<T>) -> list<T>` is accepted and executed by the Rust reference, plus `generic_list_wrapper_incompatible.zp`, where assigning the result to `text` yields the stable `variable 'wrong' expects text, got list<number>` diagnostic at line 4, column 1. This is bounded structural list evidence only; it does not establish arbitrary malformed-generic parsing, complete A3, candidate ownership, B4, or self-hosting. The Rust parser now also has a focused nesting-aware signature splitter for the accepted `map<K, V>` parameter shape: `generic_map_wrapper.zp` accepts and executes `map_keep<K, V>(map<K, V>) -> map<K, V>`, while `generic_map_wrapper_incompatible.zp` produces `variable 'wrong' expects text, got map<text,number>` at line 4, column 1. The candidate mirrors only these exact fixtures. A Rust-reference-only nested substitution probe now accepts `keep_nested<T>(option<list<T>>) -> option<list<T>>` with an inferred `option<list<number>>` call and rejects assignment to `text` with `variable 'wrong' expects text, got option<list<number>>`; this records deeper structural behavior and runtime substitution only, and the provisional candidate does not mirror this corpus yet. Scope probes now confirm that `T` is usable inside `identity<T>` and inferred calls remain accepted, while `let leaked: T = 1` is rejected at line 4, column 1 and `fn bad(value: T) -> T` is rejected at line 1, column 1 with `unknown type annotation 'T'`; these are Rust-reference-only scope boundaries and are not candidate ownership. The next bounded checkpoint adds `generic_cross_module_library.zp`, which exports `identity<T>`, and importing main fixtures: `generic_cross_module.zp` is accepted and executed, while `generic_cross_module_incompatible.zp` is rejected at line 3, column 1 with `variable 'wrong' expects text, got number`. Rust project checking now recursively collects exported function signatures from explicitly imported modules and applies the existing generic call substitution checks to the importing main module; the candidate recognizes only this exact imported `identity(…)` corpus shape. A Rust-reference-only imported-body probe statically accepts an imported `broken<T>(value: T) -> T` signature while runtime rejects its incompatible `text` return as `type mismatch for return: expected number, got text`; imported function-body checking is therefore still a deferred boundary. Namespace collision policy, aliases, non-generic cross-module inference, and full module-wide typed-IR/LSP propagation remain open. Constraint probing is also explicitly deferred: Rust rejects `fn bounded<T: number>(…)` with `invalid generic type parameter 'T: number'`, rejects `fn bounded<T extends number>(…)` with `invalid generic type parameter 'T extends number'`, and rejects the `where` form with `unknown return type annotation 'T where T: number'`; the candidate reproduces these exact three diagnostics only as a deferred corpus record. No trait-bound or constraint semantics are implemented or claimed. The explicit-call probe `identity<number>(1)` is currently accepted by the Rust project-check path but fails at runtime with `undefined variable: number`; this records that explicit generic call syntax is not implemented as a language feature and must not be inferred from the explanatory notation used elsewhere. The candidate mirrors only the static acceptance of this exact deferred fixture. Generic-class and alias probing is likewise deferred: `generic_class_deferred.zp` is statically accepted by the current Rust project-check path but fails at runtime with `unexpected token after expression at 1:6`, while `generic_alias_deferred.zp` is rejected at line 3, column 1 with `unknown type annotation 'NumberBox<number>'`. The candidate mirrors only these exact corpus boundaries; generic class instantiation, generic field metadata, alias expansion, and alias diagnostics are not implemented or claimed. A bounded typed-IR checkpoint now mirrors the Rust `generic_identity.zp` artifact exactly at the IR-node level: the generic function node carries `type_params: ["T"]`, and the two inferred call declarations carry `number` and `text` inferred types. This is a typed-IR metadata parity slice only; generic multi-parameter/wrapper IR, class/alias IR, and imported-module IR remain open. A focused Rust LSP regression now renders a generic function hover as `function `identity`<T> -> `T`` from the same declaration metadata, and a separate document-symbol regression reports `function<T> in file:///generic-symbols.zp` for the corresponding `identity` declaration. These are bounded hover and document-symbol detail surfaces; a signature-help regression now accepts the inferred `identity(1` call context and preserves the label `fn identity<T>(value: T) -> T` with parameter metadata. Explicit generic call arguments remain deferred, and imported-module LSP propagation plus the complete candidate LSP implementation remain open.
