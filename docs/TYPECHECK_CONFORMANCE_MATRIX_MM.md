...
| Generic design | Generic list/map/function syntax နှင့် inference contract | Implemented baseline | `TYPECHECK_GENERIC_DESIGN_MM.md`၊ bounded `identity<T>`/`same<T>` declaration evidence ကို သီးခြားမှတ်တမ်းတင်ထားပြီး constraint နှင့် advanced inference များကို deferred ထားသည် |

## လက်ရှိ conformance အထောက်အထား

TC-006 အတွက် guarded `while` body အတွင်း narrowed payload ကို အသုံးပြုနိုင်ပြီး loop ပြီးနောက် မူလ wrapper type ကို ပြန်လည်ရရှိကြောင်း အတည်ပြုသည့် permanent loop-boundary fixtures များ ရှိပါသည်။ Native else-branch narrowing သည် explicit `is_option_none(value)` guard ကို ထောက်ပံ့ပြီး true branch တွင် option wrapper ကို ထိန်းသိမ်းကာ else branch တွင် payload type ကို အသုံးပြုစေပါသည်။ v2.11.12 preparation line တွင် ထို direct shape အတွက် candidate-side native နှင့် Zap fixture များကို ထည့်ထားပြီး tracked `option<number>` variable တစ်ခု၊ indented else body တစ်ခုနှင့် paired acceptance/rejection diagnostic များအတွက်သာ ကန့်သတ်ထားသည်။ Publish လုပ်ပြီးသော v2.11.14 release တွင် direct bool-literal annotation pair တစ်ခု ပါဝင်ပြီး `true`/`false` ကို `bool` သို့ assign လုပ်ပါက လက်ခံကာ direct numeric literal ကို `bool` သို့ assign လုပ်ပါက reject လုပ်သည်။ ထို့အပြင် direct none-literal annotation pair တစ်ခုတွင် `none` ကို `none` သို့ assign လုပ်ပါက လက်ခံကာ direct boolean literal ကို `none` သို့ assign လုပ်ပါက reject လုပ်သည်။ Direct list-literal annotation pair တွင် exact `[1, 2]` literal ကို `list<number>` အဖြစ် လက်ခံကာ ထို literal ကို `text` သို့ assign လုပ်ပါက reject လုပ်သည်။ published v2.11.15 release တွင် exact `{"score": 7}` literal ကို `map<text,number>` အဖြစ် infer လုပ်ပြီး ထို literal ကို `text` သို့ assign လုပ်ပါက reject လုပ်သည့် direct map-literal annotation pair ကို ထည့်ထားသည်။ published v2.11.16 release တွင် exact `some(1)` expression ကို `option<number>` အဖြစ် infer လုပ်ပြီး ထို expression ကို `text` သို့ assign လုပ်ပါက reject လုပ်သည့် direct option-constructor annotation pair ကို ထည့်ထားသည်။ v2.11.18 preparation တွင် A2 expression matrix အဖြစ် exact `1 + 2` ကို `number`၊ exact `"a" + "b"` ကို `text`၊ exact `1 < 2` ကို `bool`၊ exact `true and false` ကို `bool`၊ exact `ok(1)` ကို `result<number>`၊ exact `[1 + 1, 2]` ကို `list<number>` နှင့် exact `{"score": 1 + 2}` ကို `map<text,number>` အဖြစ် သတ်မှတ်သည့် case များ ပါဝင်ပြီး reference က reject လုပ်သည့် incompatible pair များကိုသာ ထည့်ထားသည်။ Comparison expression သည် broader annotation များတွင် compatibility-accepted ဖြစ်သောကြောင့် မရှိသော negative case ကို မဖန်တီးထားပါ။ လက်ရှိ A3 checkpoint တွင် Rust-backed `identity<T>` နှင့် `same<T>` declaration fixture များ၊ AST metadata၊ inferred identity call၊ multiple-parameter substitution၊ structural `option<T>` wrapper substitution၊ conflicting substitution၊ generic return check နှင့် runtime substitution evidence များ ပါဝင်ပြီး constraint၊ explicit generic argument၊ generic class/alias နှင့် declaration coverage အပြည့်အစုံကို ဆက်လက် deferred ထားသည်။ TC-009 အတွက် compatible branches၊ မကိုက်ညီသော branch result types နှင့် bool မဟုတ်သော condition များကို `zap check --json` ဖြင့် စစ်ဆေးသည့် permanent positive/negative fixtures များ ထည့်သွင်းပြီးဖြစ်သည်။ v2.11.8 release တွင် text literal ဖြင့် index လုပ်သော bounded `map<text,number>` element အတွက် paired incompatible diagnostic ပါဝင်သည်။ v2.11.9 release တွင် tracked `option<number>` ကို indented `if` body တစ်ခုအတွင်း direct `is_some` guard ဖြင့် narrowing လုပ်သည့် paired incompatible payload assignment diagnostic ပါဝင်သည်။ v2.11.11 release တွင် indented `while` body တစ်ခုအတွင်း direct `is_some` guard ဖြင့် narrowing လုပ်ပြီး loop ပြီးနောက် မူလ `option<number>` wrapper ပြန်လည်ရရှိကြောင်း paired incompatible fixture ဖြင့် စစ်ဆေးထားသည်။ ထို့အပြင် L3 regression သည် incompatible conditional expression အတွက် JSON `ok`၊ `kind`၊ `file`၊ `line`၊ `column`၊ `message` နှင့် `error` fields များ မပြောင်းလဲဘဲ ထွက်ရှိကြောင်း အတည်ပြုပါသည်။ TC-010 အတွက် `option<T>` နှင့် `result<T>` wrapper identity သည် alias assignment ဖြတ်သန်းပြီးနောက် ထိန်းသိမ်းထားကြောင်းနှင့် reassignment ပြုလုပ်ပါက narrowed alias fact ကို invalidate လုပ်ကြောင်း အတည်ပြုသည့် permanent fixtures များ ထည့်သွင်းပြီးဖြစ်သည်။ TC-012 ကို `list<T>`၊ `map<K, V>`၊ `option<T>` နှင့် `result<T>` annotation များအတွက် implemented baseline အဖြစ် မှတ်တမ်းတင်ထားပြီး malformed generic form များကို reject လုပ်ကာ user-defined generic declaration နှင့် advanced inference များကို explicit deferred scope အဖြစ် ထားရှိထားပါသည်။ ဤ fixtures များသည် L2 behavior နှင့် TC-009 L3 diagnostic contract ကို တည်ဆောက်ပေးပါသည်။ `lsp_diagnostics_match_cli_type_error_contract` L4 regression သည် LSP diagnostics များက shared static checker ကို ပြန်လည်အသုံးပြုကြောင်း၊ `TypeError` code ကို ထိန်းသိမ်းကြောင်း၊ source location တူညီမှုကို LSP ၏ zero-based range သို့ မှန်ကန်စွာ ပြောင်းပေးကြောင်းနှင့် normalized message ကို ထုတ်ပေးကြောင်း အတည်ပြုပါသည်။ Lint diagnostics များကိုလည်း အဆိုပါ source-diagnostic bridge မှတစ်ဆင့် ဆက်လက်ရရှိနိုင်ပါသည်။
...
TC-001 မှ TC-012 အထိကို သတ်မှတ်ထားသော supported syntax boundary အတွင်း အကောင်အထည်ဖော်ပြီးဖြစ်သည်။ v2.11.7 candidate တွင် `list<list<number>>` indexing နှင့် numeric result ကို `text` သို့ assign လုပ်ပါက reject ဖြစ်ကြောင်း paired fixture များဖြင့် bounded nested-list slice တစ်ခု ထည့်ထားသည်။ v2.11.8 release တွင် tracked `map<text,number>` variable နှင့် text-literal key အတွက်သာ ကန့်သတ်ထားသော bounded map-element slice နှင့် paired valid/incompatible fixture များ ထည့်ထားသည်။ v2.11.9 release တွင် tracked `option<number>` variable ကို direct `is_some` guard ပါသော indented `if` body တစ်ခုအတွင်းသာ narrowing လုပ်သည့် bounded slice နှင့် paired valid/incompatible fixture များ ထည့်ထားသည်။ v2.11.11 release တွင် direct `is_some` guard ပါသော indented `while` body အတွက် bounded loop slice နှင့် loop ပြီးနောက် wrapper restoration ကို paired incompatible fixture ဖြင့် စစ်ဆေးထားသည်။ v2.11.12 tag attempt တွင် tracked `option<number>` variable တစ်ခုနှင့် indented else body တစ်ခုအတွက် bounded direct `is_option_none` else-body slice နှင့် paired valid/incompatible fixture များ ထည့်ထားသော်လည်း macOS ARM64 release workflow မအောင်မြင်မီ publication မဖြစ်ခဲ့ပါ။ published v2.11.13 corrective release တွင် အဆိုပါ evidence နှင့် validated cross-platform test-harness fix ကို ဆက်လက်သယ်ဆောင်ထားသည်။ Publish လုပ်ပြီးသော v2.11.14 release တွင် direct bool-literal annotation pair တစ်ခု ပါဝင်ပြီး `true`/`false` ကို `bool` သို့ assign လုပ်ပါက လက်ခံကာ direct numeric literal ကို `bool` သို့ assign လုပ်ပါက reject လုပ်သည်။ ထို့အပြင် direct none-literal annotation pair တစ်ခုတွင် `none` ကို `none` သို့ assign လုပ်ပါက လက်ခံကာ direct boolean literal ကို `none` သို့ assign လုပ်ပါက reject လုပ်သည်။ Direct list-literal annotation pair တွင် exact `[1, 2]` literal ကို `list<number>` အဖြစ် လက်ခံကာ ထို literal ကို `text` သို့ assign လုပ်ပါက reject လုပ်သည်။ published v2.11.15 release တွင် exact `{"score": 7}` map-literal annotation pair ကို stable acceptance/rejection diagnostics နှင့် ထည့်ထားသည်။ published v2.11.16 release တွင် exact `some(1)` option-constructor annotation pair ကို stable acceptance/rejection diagnostics နှင့် ထည့်ထားသည်။ v2.11.18 preparation တွင် exact arithmetic၊ text addition၊ boolean logic၊ comparison၊ result construction၊ list arithmetic နှင့် map arithmetic အတွက် A2 expression matrix ကို stable Rust-confirmed diagnostics နှင့် ထည့်ထားသည်။ လက်ရှိ A3 checkpoint တွင် `identity<T>` နှင့် `same<T>` declaration များအတွက် AST metadata၊ inferred identity call၊ multiple-parameter substitution၊ structural `option<T>` wrapper substitution၊ conflicting substitution၊ generic return check နှင့် runtime substitution evidence ကို ထည့်ထားသည်။ နောက်ထပ် gate များသည် ဤ slice များကို generalize မလုပ်ဘဲ သီးခြား evidence ဖြင့်သာ ဆက်လုပ်ရမည်။ နောက်ထပ်လုပ်ငန်းများတွင် ပိုမိုကျယ်ပြန့်သော collection inference၊ nested map၊ compound guard၊ loop mutation၊ reassignment invalidation၊ alias၊ arbitrary nested expression၊ generic constraint၊ explicit generic argument၊ generic class/alias နှင့် user-defined generic declaration အပြည့်အစုံကို design record နှင့် release gate အသစ်အောက်တွင်သာ တိုးချဲ့ရမည်။ Fixture တစ်ခုအတွက်သာ generic syntax ကို အလျင်စလို မတိုးချဲ့သင့်ပါ။
