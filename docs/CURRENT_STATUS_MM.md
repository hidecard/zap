...
| B2 type-checker candidate | provisional | Selected declaration၊ conditional၊ function၊ call၊ bounded list-element diagnostic၊ paired nested-list index slice၊ bounded text-key map-element slice၊ bounded direct-`is_some` branch-local option-narrowing slice၊ bounded direct-`is_some` loop-body/loop-boundary restoration slice နှင့် bounded direct-`is_option_none` else-body slice များ ပါဝင်သည်။ |
