...
**Release အခြေအနေ:** Candidate preparation ဖြစ်ပြီး complete validation နှင့် public artifact/signature verification ပြီးစီးမှသာ publish လုပ်မည်။ Zap သည် B0 အဖြစ်သာ ဆက်ရှိသည်။

## အကျဉ်းချုပ်

Zap v2.11.16 တွင် exact direct option-constructor annotation shape တစ်ခုအတွက် provisional၊ corpus-limited B2 type-checker increment ကို ပြင်ဆင်ထားသည်။ Candidate သည် exact `some(1)` expression ကို `option<number>` အဖြစ် သတ်မှတ်ပြီး `let selected: option<number> = some(1)` ကို လက်ခံသည်။ Paired negative fixture သည် ထို direct expression ကို `text` သို့ assign လုပ်ပါက line 1၊ column 1 တွင် `variable 'wrong' expects text, got option<number>` diagnostic ဖြင့် reject လုပ်သည်။
...
| Ownership | Provisional `BOOT-032` မှတ်တမ်းတင်ထားသည်။ | Candidate evidence သည် corpus-limited ဖြစ်သည် |
