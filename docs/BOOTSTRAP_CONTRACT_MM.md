...
**အခြေအနေ:** Published Zap v2.11.18 အတွက် B0 reference baseline
...
Repository တွင် reference-only B2 conformance gate ကို [`scripts/bootstrap/verify_b2_typecheck.sh`](../scripts/bootstrap/verify_b2_typecheck.sh) အဖြစ် ထည့်ထားသည်။ ၎င်းသည် annotated typed-IR artifact ကို native run များအကြား byte-for-byte နှိုင်းယှဉ်ပြီး annotated/conditional expression များ၏ type-check acceptance နှင့် incompatible annotation၊ function-call၊ collection-element၊ bounded map-element များ၏ rejection ကို စစ်ဆေးသည်။ Typed-IR artifact သည် `reference_only` အဖြစ် ဆက်ရှိနေပြီး ဤအရာသည် conformance foundation သာဖြစ်ကာ Zap-owned type checker မဖြစ်သေးပါ။

ပထမဆုံး provisional Zap-owned type-checker candidate ကို [`bootstrap/b2/typecheck.zp`](../bootstrap/b2/typecheck.zp) တွင် ထည့်ထားပြီး [`scripts/bootstrap/verify_b2_typecheck_candidate.sh`](../scripts/bootstrap/verify_b2_typecheck_candidate.sh) သည် annotated နှင့် conditional fixture၊ return ပါသော annotated function တစ်ခု၊ incompatible-number၊ incompatible-call၊ negative collection-element diagnostic၊ bounded nested-list index inference slice၊ bounded map-element index inference slice၊ bounded branch-local option-narrowing slice၊ bounded loop-body narrowing/loop-boundary restoration slice နှင့် bounded direct `is_option_none` else-body narrowing slice၊ bounded direct bool-literal၊ none-literal၊ exact map-literal နှင့် exact collection-expression annotation slice များ၏ deterministic behavior ကို enforce လုပ်သည်။ Nested slice သည် `list<list<number>>` indexing ကို လက်ခံပြီး numeric result ကို `text` သို့ assign လုပ်ပါက paired fixture များဖြင့် reject လုပ်သည်။ Map slice သည် text literal key ဖြင့် index လုပ်သော tracked `map<text,number>` variable အတွက်သာ ကန့်သတ်ထားပြီး numeric result ကို `text` သို့ assign လုပ်ပါက paired fixture ဖြင့် reject လုပ်သည်။ Branch slice သည် indented `if` body တစ်ခုအတွင်း direct `is_some` guard ဖြင့် narrowed လုပ်ထားသော tracked `option<number>` variable အတွက်သာ ကန့်သတ်ထားပြီး numeric payload ကို `text` သို့ assign လုပ်ပါက paired fixture ဖြင့် reject လုပ်သည်။ Loop slice သည် indented `while` body တစ်ခုအတွင်း direct `is_some` guard ဖြင့် narrowed လုပ်ထားသော tracked `option<number>` variable အတွက်သာ ကန့်သတ်ထားပြီး body အတွင်း positive use နှင့် loop ပြီးနောက် မူလ option wrapper ပြန်လည်ရရှိကြောင်း paired incompatible fixture ဖြင့် စစ်ဆေးသည်။ Else-body slice သည် direct `if is_option_none(name): ... else:` shape အောက်ရှိ tracked `option<number>` variable တစ်ခုတည်းအတွက်သာ ကန့်သတ်ထားပြီး true body တွင် wrapper ကို ဆက်ထားကာ indented else body တစ်ခုအတွင်း numeric payload ရရှိကြောင်း paired incompatible fixture ဖြင့် reject evidence ပါ စစ်ဆေးသည်။ Bool slice သည် direct `true`/`false` literal ကို `bool` annotation သို့ assign လုပ်သည့် case တစ်ခုနှင့် direct numeric literal ကို `bool` သို့ assign လုပ်ပါက reject လုပ်သည့် paired fixture တစ်ခုအတွက်သာ ကန့်သတ်ထားသည်။ None slice သည် direct `none` literal ကို `none` annotation သို့ assign လုပ်သည့် case တစ်ခုနှင့် direct boolean literal ကို `none` သို့ assign လုပ်ပါက reject လုပ်သည့် paired fixture တစ်ခုအတွက်သာ ကန့်သတ်ထားသည်။ List-literal slice သည် exact `[1, 2]` literal ကို `list<number>` အဖြစ် infer လုပ်သည့် case တစ်ခုနှင့် ထို direct literal ကို `text` သို့ assign လုပ်ပါက reject လုပ်သည့် paired fixture တစ်ခုအတွက်သာ ကန့်သတ်ထားသည်။ Map-literal slice သည် exact `{"score": 7}` literal ကို `map<text,number>` အဖြစ် infer လုပ်သည့် case တစ်ခုနှင့် ထို direct literal ကို `text` သို့ assign လုပ်ပါက reject လုပ်သည့် paired fixture တစ်ခုအတွက်သာ ကန့်သတ်ထားသည်။ Option-constructor slice သည် exact `some(1)` expression ကို `option<number>` အဖြစ် infer လုပ်သည့် case တစ်ခုနှင့် ထို direct expression ကို `text` သို့ assign လုပ်ပါက reject လုပ်သည့် paired fixture တစ်ခုအတွက်သာ ကန့်သတ်ထားသည်။ General option/result constructor inference မဟုတ်ပါ။ A2 expression matrix တွင် exact `1 + 2` ကို `number`၊ exact `"a" + "b"` ကို `text`၊ exact `1 < 2` ကို `bool`၊ exact `true and false` ကို `bool`၊ exact `ok(1)` ကို `result<number>`၊ exact `[1 + 1, 2]` ကို `list<number>` နှင့် exact `{"score": 1 + 2}` ကို `map<text,number>` အဖြစ် သတ်မှတ်သည့် case များ ပါဝင်ပြီး reference က reject လုပ်သည့် incompatible pair များကိုသာ ထည့်ထားသည်။ Comparison expression သည် broader annotation များတွင် compatibility-accepted ဖြစ်သောကြောင့် မရှိသော negative case ကို မဖန်တီးထားပါ။ ၎င်းနှင့် ကိုက်ညီသော candidate-only typed-IR producer ကို [`bootstrap/b2/typed_ir.zp`](../bootstrap/b2/typed_ir.zp) တွင် ထည့်ထားပြီး [`scripts/bootstrap/verify_b2_typed_ir_candidate.sh`](../scripts/bootstrap/verify_b2_typed_ir_candidate.sh) သည် owned node field များကို native reference artifact နှင့် နှိုင်းယှဉ်သည်။ နှစ်ခုလုံးသည် corpus-limited ဖြစ်ပြီး general expression inference၊ generic/variant narrowing၊ function checking အပြည့်အစုံနှင့် diagnostic parity အပြည့်အစုံ မလုပ်သေးပါ။ Native Rust သည် reference owner အဖြစ် ဆက်ရှိပြီး bootstrap stage သည် B0 အဖြစ်သာ ဆက်ရှိသည်။
...
လက်ရှိ implementation gate သည် staged parity expansion ဖြစ်သည်။ Zap-owned lexer နှင့် parser candidate များသည် owned corpus ကို ဆက်လက်ချဲ့ထွင်ပြီး valid၊ Unicode၊ malformed၊ overflow နှင့် determinism case များတွင် Rust reference နှင့် output နှိုင်းယှဉ်ရမည်။ B2 typed-IR/type-check conformance foundation၊ ပထမ provisional Zap-owned checker candidate နှင့် candidate-only typed-IR producer တို့ကို enforce လုပ်ပြီးဖြစ်သော်လည်း type checking နှင့် typed IR အပြည့်အစုံသည် native-owned အဖြစ်သာ ရှိသေးသည်။ v2.11.7 increment တွင် provisional list-element inference path တစ်ခုနှင့် negative collection-element diagnostic fixture တစ်ခု ထည့်ထားပြီး v2.11.8 increment တွင် ownership မပြောင်းဘဲ paired valid/incompatible fixture များပါသော bounded map-element path တစ်ခု ထည့်ထားသည်။ v2.11.9 increment တွင် direct `is_some` branch-local narrowing path တစ်ခုကို paired valid/incompatible fixture များဖြင့် ထည့်ထားသည်။ v2.11.11 increment တွင် indented `while` body တစ်ခုအတွင်း direct `is_some` narrowing နှင့် loop ပြီးနောက် wrapper restoration ကို paired incompatible fixture ဖြင့် စစ်ဆေးထားသည်။ v2.11.12 tag attempt တွင် tracked `option<number>` variable တစ်ခုအတွက် direct `is_option_none` else-body narrowing ကို paired valid/incompatible fixture များဖြင့် ထည့်ခဲ့သော်လည်း macOS ARM64 release workflow မအောင်မြင်မီ publication မဖြစ်ခဲ့ပါ။ published v2.11.13 corrective release တွင် အဆိုပါ evidence နှင့် validated cross-platform test-harness fix ကို ဆက်လက်သယ်ဆောင်ထားသည်။ published v2.11.14 release တွင် direct bool-literal၊ none-literal နှင့် exact `[1, 2]` list-literal annotation case များကို paired acceptance/rejection fixture များဖြင့် ထည့်သွင်းထားသည်။ published v2.11.15 release တွင် direct exact `{"score": 7}` map-literal annotation case ကို paired acceptance/rejection fixture များဖြင့် ထည့်သွင်းထားသည်။ published v2.11.16 release တွင် exact direct `some(1)` option-constructor annotation case ကို paired acceptance/rejection fixture များဖြင့် ထည့်သွင်းထားသည်။ v2.11.18 preparation တွင် exact list/map arithmetic expression များပါဝင်သော A2 exact-expression matrix ကို သီးခြား evidence ပါသည့် B0-safe increment အဖြစ် မှတ်တမ်းတင်ထားသည်။ လက်ရှိ A3 checkpoint တွင် Rust-backed generic `identity<T>`/`same<T>` declaration slice၊ AST metadata၊ multiple-parameter substitution၊ structural `option<T>` နှင့် `result<T>` wrapper substitution၊ generic arity diagnostic၊ inferred identity call၊ conflict diagnostic နှင့် runtime substitution check များကို ထည့်ထားသည်။ Broader collection inference၊ arbitrary nested expression၊ nested map၊ compound guard၊ loop mutation၊ reassignment invalidation၊ alias၊ generic constraint၊ explicit generic call argument၊ generic class/alias နှင့် user-defined generic declaration အပြည့်အစုံသည် သီးခြား evidence/design gate များနောက်တွင် ဆက်လက်လုပ်ဆောင်ရန် ကျန်ရှိသည်။ VM နှင့် native backend အလုပ်များကို ထို gate များမကျော်မီ ပြီးစီးသည်ဟု မဆိုရ။
