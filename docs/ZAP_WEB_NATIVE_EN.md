...
**Verified baseline:** Zap v2.11.17 on merged `master`. The original Framework work is preserved as the Web contract foundation.
...
The next CLI additions should be implemented only when their semantics are real and testable. `zap web routes` provides a resolved route-table view without starting a listener, and `zap explain route <path>` now explains concrete path matches, extracted parameters, and wildcard values without executing handlers. The roadmap remains `zap docs` for generated API documentation and `zap deploy preflight` for environment and security policy checks. The current `zap db migrate` implementation is SQLite-first and additive; it must not be mistaken for a provider-neutral production migration system.
