...
Zap ၏ လက်ရှိ verified baseline သည် v2.1.13 ဖြစ်ပါသည်။ P2 package၊ async၊ registry နှင့် editor အလုပ်များကို production-boundary limitation များကို explicit ဖော်ပြထားသော deterministic foundation များအဖြစ် track လုပ်ထားပါသည်။
...
| Async runtime နှင့် language syntax | Bounded foundation ပြီးစီး | Deterministic single-thread executor၊ `spawn`၊ `run_until_idle`၊ `block_on`၊ timers၊ cooperative cancellation၊ task limits၊ poll budgets၊ `async fn`၊ `await`၊ deterministic `Future` values နှင့် evaluator integration ကို ထည့်သွင်းပြီးဖြစ်သည်။ Executor-backed language scheduling နှင့် ပိုမိုပြည့်စုံသော task lifecycle semantics များကို deferred ထားသည်။ |
