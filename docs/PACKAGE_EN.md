...
The current registry foundation supports deterministic JSON index validation, range-based direct and transitive dependency selection, local and HTTPS index/artifact transport, content-addressed caching, SHA-256 integrity enforcement, offline reuse, checksum-verified archive publishing, and lockfile-aware cache garbage collection. Run `zap registry gc --dry-run [dir]` first to list stale artifacts and temporary files without changing them; omit `--dry-run` to remove the same candidates. The command requires a valid canonical lockfile, preserves every package referenced by that lockfile, and processes candidate paths in lexical order. When `ZAP_CACHE_DIR` points to a shared cache, run GC separately for each project that owns references; the command is intentionally project-scoped and never invents keep entries. Signed indexes, registry authentication policy, and server-side publishing persistence remain later ecosystem milestones. No network access occurs unless an explicit registry index/source or publish command is configured.
