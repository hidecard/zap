...
|---:|---|---|---|
| 1 | Direct AST call evaluation | လုပ်ဆောင်နေဆဲ | Native AST သည် literals၊ collections၊ operators၊ user-function calls၊ member access၊ object methods၊ list/map indexing နှင့် `len`၊ `range`၊ `sum`၊ `split`၊ `join`၊ `ok`၊ `some`၊ `unwrap` ကဲ့သို့သော pure built-ins များကို တိုက်ရိုက် evaluate လုပ်နိုင်ပြီ။ filesystem/time-sensitive built-in fallback များကို ဆက်လက်ပြောင်းရန် ကျန်ရှိသည် |
