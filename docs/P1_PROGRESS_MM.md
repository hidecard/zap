...
| P1-05 conformance/property/security layer runner | အခြေခံအဆင့် အကောင်အထည်ဖော်ပြီး | `scripts/test_p105_layers.sh` သည် deterministic parser၊ lexer၊ JSON၊ lockfile၊ registry-security၊ collection/filesystem၊ cancellation၊ async-scheduler နှင့် malformed source mutation ခုနစ်မျိုးကို panic မဖြစ်ဘဲ safe rejection ပြုလုပ်သော fuzz-style regression များကို run လုပ်သည်။ CI တွင် Linux၊ Windows နှင့် macOS build/test matrix coverage ကိုလည်း ဆက်လက်ထားရှိသည်။ Long-running fuzz target များ၊ allocator/heap-level counter များနှင့် platform-specific input case များကို ဆက်လက်ထည့်ရန် ကျန်ရှိသည် |

## လက်ရှိ verification baseline

Native Rust test suite သည် လက်ရှိ **test 397 ခု** အားလုံး pass ဖြစ်ပါသည်။ ၎င်းတွင် unit test 143 ခုနှင့် integration test 254 ခု ပါဝင်ပါသည်။ P1-05 deterministic layer runner သည် parser/lexer/JSON/lockfile/security corpus၊ collection regression နှင့် async cancellation/scheduler cases များကို အောင်မြင်စွာ run လုပ်သည်။ [`examples/default_parameters.zp`](../examples/default_parameters.zp) ကိုလည်း run စမ်းပြီး အောင်မြင်ပါသည်။ Type-narrowing၊ package၊ standard-library၊ benchmark နှင့် release guides များကို English/Burmese နှစ်ဘာသာဖြင့် synchronize ပြုလုပ်ထားပါသည်။ Native CLI version output သည် `v1.0.0` release line နှင့် ကိုက်ညီပါသည်။ `cargo fmt --check` နှင့် `git diff --check` လည်း pass ဖြစ်ပါသည်။ Stable Rust Clippy ကို GitHub Actions release workflow တွင် enforce လုပ်ထားပြီး local sandbox တွင် Clippy component မရှိသည့်အတွက် local result အဖြစ် မကြေညာထားပါ။
