...
| Dependency-free benchmark harness | ပြီးစီး | `scripts/benchmark_native.sh` သည် loops၊ calls၊ closures၊ allocations၊ JSON၊ async scheduling နှင့် explicit module/import dispatch များကို စမ်းသပ်ပြီး `scripts/aggregate_benchmark.sh` သည် timing threshold မသတ်မှတ်ဘဲ deterministic min/mean/max summary ထုတ်ပေးသည် |
