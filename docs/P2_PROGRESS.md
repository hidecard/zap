...
Zap's current verified baseline is v2.2.0. P2 package, async, registry, and editor work is tracked as deterministic foundations with explicit production-boundary limitations.
...
| Standard-library stability policy | Implemented as M3-STDLIB-01 | The machine-readable catalog and bilingual policy define stability labels, introduction releases, deprecation windows, semver rules, release-target platforms, limits, timeout/error contracts, and determinism metadata for all twelve public domains and every cataloged builtin. Catalog and policy contract regressions run in CI and release preflight. |
| LSP/editor integration | Implemented as M3-LSP-01 | `zap lsp` provides stdio JSON-RPC framing, initialize/shutdown, text synchronization, parser-backed diagnostics, parser-span hover and definitions, context-aware completion/signature help, deterministic formatting, parser/lexer-backed rename edits, didClose workspace cleanup, nested/module-aware workspace-symbol indexing, and catalog-driven completion. Async builtin hover/signature metadata, a checked-in VS Code grammar/configuration package, an editor asset validator, and CI/release-preflight semantic-parity enforcement are included. |
