...
**Baseline:** Zap v2.2.0 remains the verified published release. Its tag is immutable; the later LSP, VS Code, and standard-library corrections are post-release `master` work tracked toward v2.2.1.
...
| M2-MEM-02 | Memory lifecycle and statistics hardening | Complete deterministic object charges, validation/cleanup lifecycle counters, output/task admission, reset detachment, and stable memory diagnostics | `memory_stats()` budget/lifecycle fields, `ZAP-MEMORY-001`, repeated module-cache behavior, focused regressions, and full native suite |
...
| M2-FN-02 | Parent-linked EnvFrame closures | Replace function closure snapshots with parent-linked lexical frames and define lookup, mutation, recursion, and returned-closure lifetime semantics | Returned-closure mutation regression, callable higher-order tests, deterministic type/arity diagnostics, bilingual memory/spec contracts, and full native suite |
| M2-ASYNC-01 | Executor-backed language scheduling | Schedule language async results through the caller's `RuntimeState`; expose deterministic readiness, join, poll, task admission, and reset semantics | ScheduledFuture AST tests, scheduler isolation/reset regression, bilingual async-boundary contract, and full native suite |
| M2-ASYNC-02 | Cooperative language task cancellation and timeout | Add context-owned cancellation requests and deterministic poll-budget timeout joins with stable task errors | `task_cancel`/`task_join_timeout` AST and end-to-end regressions, cancellation/timeout diagnostics, bilingual async documentation, and full native suite |
...
| 13 | **M4-REL-01 v2.2.0 release** | Published and verified | Cargo-authoritative versioning, bilingual release notes, signed cross-platform assets, checksums/provenance, and successful GitHub Actions verification | M4 — Release |
...
| M1-04-06 | Replace object-field `RefCell` panics with checked borrow access and stable diagnostics | `try_borrow`/`try_borrow_mut`, fallible object-field helpers, `ZAP-BORROW-001`, JSON propagation, and panic-free conflict regressions |
...
The documentation milestone is complete: a beginner can learn without reading internals, an experienced user can find normative syntax and error behavior, and an operator can identify deployment responsibilities without confusing a local fixture with a public production service. The checked-surface matrix and bilingual navigation hubs are verified for v2.2.0.
...
No parser or runtime implementation is enabled by this milestone. Traits, interfaces, composition, and method-resolution syntax remain deferred beyond v2.2.0 until a later implementation proposal passes the listed conformance and specification gates.
...
| v2.2.1 | Planned | New patch tag and release assets from a clean version-consistent commit; v2.2.0 will not be retagged or rewritten. |
