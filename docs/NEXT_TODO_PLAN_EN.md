...
| M2-MEM-02 | Memory lifecycle and statistics hardening | Implemented logical accounting slice: deterministic nested-value, callable-capture/default, builtin-output, and finalized-object charges with byte/output rollback for failed AST expressions, builtins, and constructors; task lifecycle remains a separate PR-5 gate | `memory_stats()` budget/lifecycle fields, `ZAP-MEMORY-001`, repeated module-cache behavior, focused value/evaluator regressions, and full native suite |
