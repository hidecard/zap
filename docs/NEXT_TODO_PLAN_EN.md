...
**Baseline:** Zap v2.2.6 is the current release candidate following the verified published v2.2.4 release. The v2.2.0, v2.2.1, and v2.2.2 tags and assets remain immutable; the later runtime, equality, borrow, and LSP hardening changes are published in v2.2.3, while broader deferred work remains on the roadmap.
...
| v2.2.6 | Prepared for release | HTTP URL parser-result, scheme, and host invariant handling now returns deterministic errors instead of panicking; parser/runtime syntax, eager async behavior, and framework scope are unchanged. |
