...
| M2-ASYNC-02 | Cooperative language task cancellation and timeout | Implemented stable cancellation/timeout terminal transitions and deterministic `UnknownTask`/`AlreadyJoined` diagnostics; lazy-versus-eager async body execution remains a separate PR-6 semantic slice | `task_cancel`/`task_join_timeout` AST and end-to-end regressions, cancellation/timeout diagnostics, one-time task-budget-release regression, bilingual async documentation, and full native suite |
