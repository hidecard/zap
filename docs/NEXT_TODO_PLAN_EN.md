...
| v2.2.6 | Candidate prepared; publication held | HTTP URL parser-result, scheme, and host invariant handling now returns deterministic errors; Unix direct/async process-group cleanup uses an explicit `kill` option terminator with focused regressions; six RustSec advisories remain in the locked graph, so no tag or release is published. Parser/runtime syntax, eager async behavior, and framework scope are unchanged. |
