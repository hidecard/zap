...
| v2.2.6 | Candidate prepared; dependency remediation verified; publication pending final CI/release gates | HTTP URL parser-result, scheme, and host invariant handling now returns deterministic errors; Unix direct/async process-group cleanup uses an explicit `kill` option terminator with focused regressions; the isolated remediation branch updates the locked graph to `ureq 2.12.1`, `url 2.5.8`, `idna 1.1.0`, `rustls 0.23.40`, `rustls-webpki 0.103.15`, `rcgen 0.13.2`, and dev-only `time 0.3.47`. Strict `cargo-audit 0.22.2` reports zero unresolved advisories across 87 locked crate dependencies. Rust 1.88.0 is required by `time 0.3.47`; clean commit, GitHub CI, final preflight, and published artifact verification remain. Parser/runtime syntax, eager async behavior, and framework scope are unchanged. |
