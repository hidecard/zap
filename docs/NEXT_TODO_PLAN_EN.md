...
**Baseline:** Zap v2.2.6 is the historical release-candidate baseline for this legacy planning record, following the verified published v2.2.5 release. The candidate commit is `efe44a621251a1d61e85480fced6593b9bd27941`; no v2.2.6 tag or release exists in that historical record. The v2.2.0, v2.2.1, and v2.2.2 tags and assets remain immutable; the later runtime, equality, borrow, and LSP hardening changes are published in v2.2.3, while broader deferred work remains on the roadmap.

**Current status correction (2026-09-01):** This document’s v2.2.6 wording is historical and is not the repository’s current release status. The latest published release is v2.11.18. The v2.11.12 tag is preserved as an immutable failed macOS ARM64 release-workflow record with no public GitHub Release; v2.11.13 contains the validated cross-platform test fix and its workflow/artifact verification passed. v2.11.14 through v2.11.18 have since been published; v2.11.18 adds the A2 exact-expression matrix and the bounded canonical AST closure slice. The next active line is v2.11.18, and current `master` is B4 candidate with B4 verification infrastructure complete but no Zap-produced Rust-free seed binary yet. Rust remains the complete/reference compiler and runtime owner.
