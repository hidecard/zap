...
**Current status correction (2026-08-24):** This document’s v2.2.6 wording is historical and is not the repository’s current release status. The latest published release is v2.11.13. The v2.11.12 tag is preserved as an immutable failed macOS ARM64 release-workflow record with no public GitHub Release; v2.11.13 contains the validated cross-platform test fix and its workflow/artifact verification passed. The next active line is v2.11.14, and current `master` remains B0 with Rust as the complete/reference compiler and runtime owner. Zap remains B0 and Rust remains the complete/reference compiler and runtime owner.
