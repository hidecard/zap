...
| M2-ASYNC-01 | Executor-backed language scheduling | Language async result များကို caller ၏ `RuntimeState` မှတစ်ဆင့် schedule လုပ်ပြီး deterministic readiness၊ join၊ poll၊ task admission နှင့် reset semantics ပေးရန် | ScheduledFuture AST test၊ scheduler isolation/reset regression၊ bilingual async-boundary contract နှင့် full native suite |
| M2-ASYNC-02 | Cooperative language task cancellation နှင့် timeout | Context-owned cancellation request နှင့် stable task error ပါသော deterministic poll-budget timeout join ကို ထည့်သွင်းရန် | `task_cancel`/`task_join_timeout` AST နှင့် end-to-end regression၊ cancellation/timeout diagnostic၊ bilingual async documentation နှင့် full native suite |
