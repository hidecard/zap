...
| M2-MEM-02 | Memory lifecycle နှင့် statistics hardening | Logical accounting slice အကောင်အထည်ဖော်ပြီး — nested value၊ callable capture/default၊ builtin output နှင့် finalized object charge များကို deterministic ပြုလုပ်ကာ AST expression၊ builtin နှင့် constructor မအောင်မြင်မှုများအတွက် byte/output rollback ထည့်သွင်းထားသည်။ Task lifecycle သည် သီးခြား PR-5 gate ဖြစ်သည် | `memory_stats()` budget/lifecycle field များ၊ `ZAP-MEMORY-001`၊ repeated module-cache behavior၊ focused value/evaluator regression နှင့် full native suite |
