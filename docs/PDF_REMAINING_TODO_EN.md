...
| P1-05 | Conformance/property/fuzz test layers | Partial | Parser golden-style unit tests, deterministic parser/lexer/JSON/lockfile/registry-security corpora, collection regressions, and async cancellation/scheduler cases are now CI-visible through `scripts/test_p105_layers.sh`. Linux, Windows, and macOS build/test matrix coverage remains active. Dedicated fuzz targets, broader allocation/heap regressions, and expanded platform-specific input cases remain to be added. |
