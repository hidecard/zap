...
| P0-05 | Deterministic versus production async boundary | Partial — M2-ASYNC-01 scheduling implemented | The deterministic executor, context-owned `ScheduledFuture` language handles, fixed-worker, bounded network/process adapters, cancellation behavior, descriptive `async_capabilities()` report, typed resource-limit preflight validation, TCP request-size admission check, and reproducible Linux/Windows/macOS focused matrix with target-named CI artifacts are documented and exposed. Language-level cancellation/timeout controls, full reactor semantics, and tooling synchronization remain. |
