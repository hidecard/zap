...
`frontend_contract.zp` defines the Zap-owned browser boundary. `frontend_asset_manifest()` declares a `public` root and records that the browser runtime does not require Node. `web_static(asset_path, root_dir)` is a filesystem-capability-gated builtin that returns a normal response map for UTF-8 HTML, CSS, JavaScript/ES modules, JSON, SVG, images, fonts, Wasm, or text files. It confines the root and resolved file to the Zap workspace, rejects absolute paths, traversal components, encoded traversal, backslashes, unsupported extensions, and files larger than the bounded asset limit. Text files are returned directly and binary files use a bounded base64 response boundary; missing or unsupported assets return a deterministic `404`, while unsafe or unreadable paths fail closed.

`web_static_spa(asset_path, root_dir, fallback)` serves the requested asset when it exists and otherwise serves the validated fallback document. This keeps client-side React/Vue/Svelte routes explicit. The route matcher supports a final `*name` wildcard, so `/assets/*path` can serve nested bundle paths such as `/assets/chunks/app.js`, while a final `/*path` route can serve the SPA entry document. The wildcard is only a route parameter; the static builtins still perform canonicalization and root confinement.
