...
Before publication, the v2.2.6 release candidate must pass strict formatting, warnings-denied Clippy, locked compilation, the complete native unit and integration suites, filesystem/process/network/security corpora, project and lockfile tests, LSP and VS Code parity, documentation consistency, standard-library policy, ownership/parity/replay/async matrices, packaging, and clean tagged-name release preflight. Verification statements in this section must be updated only after the corresponding local and GitHub gates complete. The release preflight and tag-triggered release workflow now enforce a modern RustSec audit through `scripts/check_rustsec_audit.sh` and `RUN_CARGO_AUDIT=1`. The current locked graph has recorded advisories involving `idna 0.4.0`, `rustls-webpki 0.102.8`, and development-only `time 0.3.30`; dependency remediation is not claimed unless the audit passes under the repository's explicit lockfile policy. The current audit still finds six advisories, so this candidate is not publishable.
