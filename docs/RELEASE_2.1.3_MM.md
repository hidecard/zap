...
Release pipeline တွင် dry-run-first version/changelog bump helper၊ tag-gated release preflight၊ deterministic three-target artifact aggregation၊ per-artifact နှင့် aggregate SHA-256 verification၊ `zap.release-manifest.v1`၊ `zap.provenance.v1`၊ detached GPG signatures၊ `zap-2.1.3-release-signing-key.asc` public signing-key asset နှင့် post-publish release verification တို့ ပါဝင်ပါသည်။ Unix archive များအတွက် repository-owned platform-neutral deterministic tar.gz helper ကို အသုံးပြုပြီး Windows ZIP entry များအတွက် ZIP က ထောက်ပံ့သော အနိမ့်ဆုံး timestamp နှင့် မှန်ကန်သော top-level `zap/` directory ကို အသုံးပြုသဖြင့် reproducible packaging နှင့် release verification ကို တစ်သမတ်တည်း ထိန်းသိမ်းထားပါသည်။ Release incident များအတွက် bilingual rollback/quarantine runbook ကိုလည်း ထည့်သွင်းထားပါသည်။

## Supported Targets

| Platform | Target | Artifact |
|---|---|---|
| Linux | x86_64 GNU | `zap-2.1.3-linux-x86_64.tar.gz` |
| macOS | ARM64 | `zap-2.1.3-macos-arm64.tar.gz` |
| Windows | x86_64 MSVC | `zap-2.1.3-windows-x86_64.zip` |
...
GNUPGHOME=/secure/verification/gnupg \
  bash scripts/verify_published_release.sh 2.1.3 ./published-release
```
